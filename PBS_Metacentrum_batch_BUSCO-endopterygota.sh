...
outDir=${2}BUSCO_endo_${timeStamp}
mkdir $outDir
cd $outDir
...
do
	...
	scriptName=${outDir}/BUSCO_endo_"${fileName}".pbs
	(
	cat << endOfPBSscript
#!/bin/bash
#PBS -N BUSCO-endo
...
#PBS -o ${outDir}/${fileName}_BUSCO-endo.stdout
#PBS -e ${outDir}/${fileName}_BUSCO-endo.stderr

trap 'clean_scratch' TERM EXIT

scp ${file} \${SCRATCHDIR} || exit 1
cp -r /software/busco/3.0.2/src/db/endopterygota_odb9/ \${SCRATCHDIR} || exit 1
...
run_BUSCO.py -c 1 -i \${SCRATCHDIR}/${fileName}.fasta -o ${fileName}_endo_busco.out -l \${SCRATCHDIR}/endopterygota_odb9/ -m geno
